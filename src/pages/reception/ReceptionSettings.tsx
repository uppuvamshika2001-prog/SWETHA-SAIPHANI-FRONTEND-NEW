...
import { Settings, User, Lock, Bell, Shield, Mail, Phone, Building, Eye, EyeOff } from "lucide-react";
...
export default function ReceptionSettings() {
    ...
    useEffect(() => {
        if (profile) {
            ...
            setPhone("9876543210"); // Mock phone if not in profile, ideally should come from profile
        }
    }, [profile]);
    ...
    const handleProfileUpdate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await api.updateProfile({
                firstName,
                lastName,
                phone, // Ensure backend handles phone updates
            });
                ...
                <div className="grid gap-8 lg:grid-cols-[300px_1fr]">
                    ...
                    <div className="space-y-6">
                        <Card className="border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                            <div className="h-24 bg-gradient-to-r from-teal-500/10 to-blue-500/10"></div>
                                ...
                                <div className="flex justify-center mb-4">
                                    <Avatar className="h-24 w-24 border-4 border-white dark:border-slate-900 shadow-md">
                                        <AvatarFallback className="bg-teal-100 text-teal-700 text-3xl font-semibold">
                                            {profile ? getInitials(profile.full_name) : 'R'}
                                        </AvatarFallback>
                                    </Avatar>
                                </div>
                                <h2 className="text-xl font-bold text-slate-900 dark:text-white">
                                    {profile?.full_name || 'Receptionist'}
                                </h2>
                                <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">{user?.email}</p>

                                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-teal-50 text-teal-700 dark:bg-teal-900/30 dark:text-teal-300 text-xs font-medium border border-teal-100 dark:border-teal-800">
                    ...
                    <div className="space-y-6">
                        <Tabs defaultValue="security" className="w-full">
                            <TabsList className="w-full justify-start h-12 bg-transparent border-b border-slate-200 dark:border-slate-800 rounded-none p-0 mb-6 space-x-6">
                                <TabsTrigger
                                    value="profile"
                                    className="data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-teal-600 rounded-none px-0 pb-3 text-slate-500 data-[state=active]:text-teal-700 font-medium text-base transition-all"
                                >
                                        ...
                                        <form onSubmit={handleProfileUpdate} className="space-y-6">
                                            <div className="grid gap-6 md:grid-cols-2">
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    <Input id="department" defaultValue="Front Desk" disabled className="h-11 bg-slate-50 text-slate-500" />
                                                </div>
                                            </div>
                                    ...
                                    <CardContent className="pt-6">
                                        <form onSubmit={handlePasswordChange} className="space-y-6">
                                            <div className="space-y-2.5">
                                                ...
                                                <div className="relative">
                                                    <Input
                                                        ...
                                                        className="h-11 border-slate-200 focus:border-teal-500 focus:ring-teal-500 pr-10"
                                            ...
                                            <div className="flex justify-end pt-6">
                                                <Button type="submit" disabled={isLoading} className="bg-teal-600 hover:bg-teal-700 text-white shadow-sm min-w-[160px] h-11 text-base">
                            ...
                            <TabsContent value="notifications" className="mt-0 space-y-6">
                                <Card className="border-slate-200 dark:border-slate-800 shadow-sm">
                                    ...
                                    <CardContent className="pt-6 space-y-4">
                                        <div className="flex items-center justify-between p-5 border border-slate-100 dark:border-slate-800 rounded-xl bg-slate-50/50 dark:bg-slate-900/50 hover:border-teal-200 transition-colors">
                                            ...
                                            <Button variant="outline" size="sm" className="text-teal-700 border-teal-200 bg-teal-50 hover:bg-teal-100 hover:text-teal-800 min-w-[100px]">Enabled</Button>
                                        </div>
                                        <div className="flex items-center justify-between p-5 border border-slate-100 dark:border-slate-800 rounded-xl bg-slate-50/50 dark:bg-slate-900/50 hover:border-teal-200 transition-colors">
                                            <div className="space-y-1">
                                                <h4 className="font-semibold text-slate-800">System Alerts</h4>
                                                <p className="text-sm text-slate-500">Get notified about system maintenance and downtime</p>
                                            </div>
                                            <Button variant="outline" size="sm" className="text-teal-700 border-teal-200 bg-teal-50 hover:bg-teal-100 hover:text-teal-800 min-w-[100px]">Enabled</Button>
                                        </div>
                                    </CardContent>
